...
public class MapGenCavesGlacios extends MapGenBase {
    /*
    ...
    protected void generateLargeCaveNode(long par1, int par3, int par4, byte[] par5ArrayOfByte, double par6, double par8, double par10) {
        this.generateCaveNode(par1, par3, par4, par5ArrayOfByte, par6, par8, par10, 1.0F + this.rand.nextFloat() * 8.0F, 0.0F, 0.0F, -1, -1, 0.5D);
    }

    /*
     * Generates a node in the current cave system recursion tree.
     */
    protected void generateCaveNode(long par1, int par3, int par4, byte[] par5ArrayOfByte, double par6, double par8, double par10, float par12, float par13, float par14, int par15, int par16,
            double par17) {
        double d4 = par3 * 16 + 8;
        double d5 = par4 * 16 + 8;
        float f3 = 0.0F;
        ...
        for (boolean flag1 = random.nextInt(6) == 0; par15 < par16; ++par15) {
            ...
            if (!flag && par15 == k1 && par12 > 1.0F && par16 > 0) {
                this.generateCaveNode(random.nextLong(), par3, par4, par5ArrayOfByte, par6, par8, par10, random.nextFloat() * 0.5F + 0.5F, par13 - ((float) Math.PI / 2F), par14 / 3.0F, par15, par16,
                        1.0D);
                this.generateCaveNode(random.nextLong(), par3, par4, par5ArrayOfByte, par6, par8, par10, random.nextFloat() * 0.5F + 0.5F, par13 + ((float) Math.PI / 2F), par14 / 3.0F, par15, par16,
                        1.0D);
                return;
                                ...
                                if (d12 * d12 + d13 * d13 < 1.0D) {
                                    for (int k4 = k2 - 1; k4 >= j2; --k4) {
                                        ...
                                        if (d14 > -0.7D && d12 * d12 + d14 * d14 + d13 * d13 < 1.0D) {
                                            ...
                                            if (b0 == Block.blockSnow.blockID) {
                                                flag3 = true;
                                            }

                                            if (b0 == GlaciosBlocks.slate.blockID || b0 == GlaciosBlocks.glacite.blockID || b0 == Block.blockSnow.blockID) {
                                                if (k4 < 10) {
                                                    par5ArrayOfByte[j4] = (byte) Block.lavaMoving.blockID;
                                                } else {
                                                    par5ArrayOfByte[j4] = 0;

                                                    if (flag3 && par5ArrayOfByte[j4 - 1] == GlaciosBlocks.glacite.blockID) {
